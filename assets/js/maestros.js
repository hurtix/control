...
function mostrarTrazabilidadFormateada(trazabilidad) {
  ...
  const infoLoteHtml = `
    <div class="lote-info-general card p-4 mb-8 w-fit">
...
Object.keys(despachosPorFecha).forEach(fecha => {
  ...
  if (trazabilidad.recepciones && trazabilidad.recepciones.length > 0) {
    ...
    trazabilidad.recepciones.forEach(recepcion => {
      content += `<div><strong>${recepcion.tienda}</strong><br>` +
        `<strong>Fecha y Hora:</strong> ${new Date(recepcion.fecha).toLocaleString('es-ES')}<br>` +
        `<strong>Empleado:</strong> ${recepcion.empleado || 'No especificado'}<br>` +
        (recepcion.observaciones ? `<strong>Observaciones:</strong> ${recepcion.observaciones}<br>` : '') +
        `<strong>Productos Recibidos:</strong><ul style="margin:0; padding-left:1.5em;">` +
        recepcion.items.map(item => {
          const confirmadomText = item.confirmado ? '✅ Confirmado' : '❌ No confirmado';
          return `<li><strong>${item.producto}</strong>: <span>${item.cantidad_recibida} unidades</span> - ${confirmadomText}</li>`;
        }).join('') +
        `</ul></div>`;
    });
    steps.push({
